...
import { convertCurrency, generateWithoutStripe, savePatient } from '@/lib/helper';
    ...
    useEffect(() => {
        ...
        for (const property in datas.symbols) {
            if (datas.symbols.hasOwnProperty(property)) {
                symbol.push({ country: datas.symbols[property as keyof typeof datas.symbols], code: property });
            }
        }
...
const Amount: React.FC<AmountDataProps> = ({
    ...
    const convertAutomatically = async (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    const saveGenerateMutation = useMutation(generateWithoutStripe, {
        onSuccess: (res) => {
            ...
            if (res.error) {
                ...
                dispatch(setPatientDispatch({ ...patient, stripePaymentId: res.stripePaymentId }));
            ...
            <Form className="flex justify-center w-full  py-4 items-end">
                <div className="grid lg:grid-cols-2 lg:px-10 gap-6">
                    ...
                    <div className="mt-6 bg-gray-50 px-4 pt-8 lg:mt-0">
                        ...
                        >
                            {
                                saveGenerateMutation.isLoading ? 'Pass santé en cours...' : 'Passer au paiement'
                            }
                        </button>
                    </div>
                </div>
