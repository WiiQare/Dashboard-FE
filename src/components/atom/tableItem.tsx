...
const TableItems: FC<TableItemsProps<any>> = ({
  data,
  columns,
  groups,
  currentPage,
  ...
  const renderCurrency = (params: any) => {
    ...
    return (
      <div
        title={country.code(value.toString())?.currency}
        ...
        <div className="table min-w-full overflow-hidden">
          <GlobalStyles
            styles={{
              '&.MuiDataGrid-menuList, .MuiDataGrid-panel': {
                backgroundColor:
                  theme.theme === 'dark' ? '#0f172a !important' : 'white',
          ...
          <DataGrid
            sx={{
              '& .MuiButton-text': {
                color:
                  theme.theme === 'dark' ? '#ffffff' : '#1976d2 !important',
              },
            ...
            columns={columns.map((column) => ({
              ...
              renderCell: (params) => {
                ...
                if (
                  ...
                  (field.toLowerCase().endsWith('currency') ||
                    field === 'currency')
                ) {
